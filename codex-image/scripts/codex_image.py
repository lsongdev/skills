...
def get_token_auto(no_login=False) -> str:
    ...
    if no_login:
        cmd.append("--no-login")
    # T172: do NOT capture stdout. get_auth.py invokes the system `open`
    # command to show the login page so the user can log in. capture_output=True
    # would swallow stdout so the user never sees the login page and the script
    # polls uselessly for 5 minutes. Pass stdout through; success signal comes
