...
Fetches the accessToken from https://chatgpt.com/api/auth/session via browser_use.
...
def browser(action: str, **kwargs) -> dict:
    """Run a browser_use action and return parsed JSON output."""
    # Build CLI args from action + kwargs
    cmd = ["browser_use", action]
...
def main():
    ...
    # 4. Open login page in foreground via the system `open` (shows to user)
