...
def get_token():
    if os.path.exists(AUTH_CACHE):
        try:
            ...
            pass
    # T172: stdout pass-through so the login page opened via the system `open`
    # command stays visible to the user while the script polls for login.
    # capture_output would swallow stdout and the script would poll fruitlessly
    # for 5 min.
