...
5. **Auto login flow** — fetches from `https://chatgpt.com/api/auth/session` via `browser_use`:
...
1. **Token**: auto-resolved via `get_auth.py`:
   ...
   - Not logged in → `open` pops the ChatGPT login page to the user in foreground, then polls every 10s (up to 5 min) until login is detected
