...
### 🍎 iOS Alternative — browser_use validate

`npx hyperframes validate` requires headless Chrome (unavailable on iOS). Use browser_use for runtime validation:

```
1. browser_use navigate → <workspace>/<project>/index.html
2. browser_use wait_for_dom_stable
3. browser_use execute_js:
     return {
       playerReady:  !!window.__playerReady,
       renderReady:  !!window.__renderReady,
       timelines:    Object.keys(window.__timelines || {}),
       duration:     window.__hf?.duration,
       clipManifest: (window.__clipManifest || []).length,
     }
   → verify playerReady=true, renderReady=true, timelines non-empty, duration > 0
4. browser_use execute_js:
     // Check for missing assets (404s recorded by runtime)
     return window.__hf_errors || []
```

`npx hyperframes lint` (static HTML check) still works normally via shell_execute — run it first.
...
### 🍎 iOS Alternative — browser_use snapshot

`npx hyperframes snapshot` requires Puppeteer. Use `browser_use` with `set_viewport` to get accurate snapshots at the correct composition resolution:

```bash
# 1. Set viewport to composition size (MUST do before navigate)
...
# 3. Wait for runtime
browser_use execute_js --script 'return !!window.__hf'

# 4. For each beat midpoint T:
browser_use execute_js --script 'window.__hf.seek(<T>)'
browser_use screenshot   # saved to <browser>/screenshot_*.jpg

# 5. Reset viewport when done
browser_use set_viewport --reset
```

Without `set_viewport`, the browser uses the phone's default viewport (~390px wide) and the composition renders at wrong scale. Always set viewport first.
