...
### 🍎 iOS Alternative — Use browser_use instead of CLI

`npx hyperframes capture` requires a local Node.js + Puppeteer environment. Instead, use `browser_use` tool calls directly — equivalent output, no CLI needed.

**Step-by-step replacement:**

1. **Navigate & scroll-screenshot** (replaces `screenshots/scroll-*.png`):
   ```
   browser_use navigate → URL
   browser_use screenshot → scroll-000.png
   browser_use scroll(down) + screenshot → scroll-001.png  (repeat until bottom)
   ```

2. **Extract design tokens** (replaces `extracted/tokens.json`):
   ```
   browser_use execute_js:
     const els = [...document.querySelectorAll('*')];
     const colors = [...new Set(els.flatMap(el => {
       const s = getComputedStyle(el);
       return [s.color, s.backgroundColor];
     }).filter(c => c && !c.includes('rgba(0, 0, 0, 0)')))];
     const fonts = [...new Set(els.map(el => getComputedStyle(el).fontFamily))];
     const sections = document.querySelectorAll('section, [class*="section"]').length;
     return { colors, fonts, sections };
   ```

3. **Extract visible text** (replaces `extracted/visible-text.txt`):
   ```
   browser_use get_text  (or get_readable for structured content)
   ```

4. **Download assets** (replaces `assets/` directory):
   ```
   browser_use execute_js → collect all img[src], video[src], og:image URLs
   shell_execute: wget/curl each asset URL into captures/<project>/assets/
   ```

5. **Check animations** (replaces `extracted/animations.json`):
   ```
   browser_use execute_js:
     return {
       cssAnimations: [...document.styleSheets].flatMap(s => {
         try { return [...s.cssRules].filter(r => r.type === CSSRule.KEYFRAMES_RULE)
           .map(r => r.name); } catch { return []; }
       }),
...
Produce the same gate summary as below using this data.
