#!/usr/bin/env python3
"""
hf-render.py — HyperFrames Renderer
使用 browser_use set_viewport 将浏览器精确设为合成尺寸，
...
示例:
  python3 hf-render.py ./my-video
  python3 hf-render.py ./my-video --fps 30 --quality high
"""
...
# ── CLI 参数 ───────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description='HyperFrames Renderer')
...
# ── 工具函数 ───────────────────────────────────────────────────────────────────
def mbu(*cmd_args):
    """调用 browser_use，返回 data 字段。出错时打印并退出。"""
    r = subprocess.run(
        ['browser_use'] + list(cmd_args) + ['--compact'],
        capture_output=True, text=True
    )
    if r.returncode != 0:
        print(f"[ERROR] browser_use {cmd_args[0]}:\n{r.stderr}", file=sys.stderr)
        sys.exit(1)
...
def project_url():
    """
    将项目路径转换为浏览器可导航的本地 URL。
    支持任意目录下的项目（目录含 index.html 即可）。
    """
    return INDEX.as_uri()
