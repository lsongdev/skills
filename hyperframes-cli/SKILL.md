...
version: 1.0.0
description: >
  HyperFrames CLI and rendering. Use for: (1) CLI commands — init, lint, preview,
  render, transcribe, tts, doctor; (2) iOS rendering via hf-render.py
  (set_viewport + browser_use + ffmpeg, no Chrome required); (3) Website-to-video
...
source: |
  ...
  Adaptation: Merged hyperframes-cli + website-to-hyperframes into a single skill,
  added hf-render.py (iOS-native renderer using browser_use set_viewport + ffmpeg),
  added check-env.py and install-skill.py for dependency management.
  Adapter copyright: Open-source contributors, MIT License.
---
...
```bash
python3 ~/.agents/skills/hyperframes-cli/scripts/check-env.py
```

This checks:
- `browser_use` — built-in tool for browser control (required for rendering and preview)
...
```bash
python3 ~/.agents/skills/hyperframes-cli/scripts/check-env.py --fix
```

Or install a specific skill manually:
```bash
# From heygen-com/hyperframes (HyperFrames skills)
python3 ~/.agents/skills/hyperframes-cli/scripts/install-skill.py \
  hyperframes --repo heygen-com/hyperframes --subdir skills

# From a community skills repository
python3 ~/.agents/skills/hyperframes-cli/scripts/install-skill.py notion-hub
```

**If `browser_use` is missing**: it is a built-in tool. Update the app to the latest version — it ships with the app and cannot be installed separately.
...
### 🍎 iOS Alternative — Preview & Snapshot
...
browser_use set_viewport --width 960 --height 540
browser_use navigate --url <workspace>/<project>/index.html

# Snapshot at specific timestamps
browser_use execute_js --script 'window.__hf.seek(2.5)'
browser_use screenshot   # saves to <browser>/
```
...
`npx hyperframes render` requires Puppeteer + Chrome (unavailable on iOS). Use the iOS-native renderer instead — it uses `browser_use set_viewport` to lock the browser to the exact composition size, captures frames at Retina resolution, and encodes with FFmpeg.

**One-line render:**
```bash
python3 ~/.agents/skills/hyperframes-cli/hf-render.py <project_dir>
```
...
python3 ~/.agents/skills/hyperframes-cli/hf-render.py <project_dir> \
  --fps 24 \
  --quality high \
  --output <workspace>/<project>/renders/output.mp4
```
...
2. Navigate to `<workspace>/<rel-path>/index.html` (supports subdirectories)
...
- Project must be under `<workspace>/` — the renderer converts the path to a file URL
- **Multi-scene layout: use `position:absolute; inset:0` per scene** — see note below

> **⚠️ Layout difference from standard HyperFrames:**
> The standard skill says `.scene-content` should use `width:100%; height:100%`. In iOS
