...
从 GitHub 仓库下载并安装 skill 到 ~/.agents/skills/
...
默认仓库: 由 SKILLS_DEFAULT_REPO 环境变量指定（未设置时需用 --repo 指定）
...
DEFAULT_REPO = os.environ.get('SKILLS_DEFAULT_REPO', '')
DEFAULT_BRANCH = 'main'

parser = argparse.ArgumentParser(description='Skill Installer')
...
def main():
    if not args.repo:
        print("[ERROR] 未指定仓库：请用 --repo <owner/repo> 指定，或设置 SKILLS_DEFAULT_REPO 环境变量", file=sys.stderr)
        sys.exit(1)
