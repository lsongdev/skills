---
name: luckin-coffee
description: Use when users ask to order Luckin Coffee, search Luckin stores/products, query pickup code/order status, cancel a Luckin order, or mention 瑞幸、luckin、咖啡、拿铁、美式、生椰拿铁、橙C美式、果茶、轻乳茶、果蔬茶、柠檬茶、点单、下单、点一杯、喝一杯、买一杯、门店、取餐码.
keywords:
  - 瑞幸
  - luckin
  - 咖啡
...
**必需 MCP Server**: `luckin-coffee`

优先使用名为 `luckin-coffee` 的 MCP server；若当前智能体暴露的是同一瑞幸订单 MCP 的其它别名，以实际可用 server 名为准。
...
- `LUCKIN_MCP_TOKEN` 读取优先级：环境变量 `LUCKIN_MCP_TOKEN` > 当前对话用户明确提供的 token > 本地文件 `~/.luckin-coffee/LUCKIN_MCP_TOKEN`（仅在用户明确同意记录后可使用）。
- 如果用户在当前或历史消息里发过完整 token，应先尝试该 token，不要直接让用户重新登录平台获取。
- 用户发送 token 时必须先询问是否记录到 `~/.luckin-coffee/LUCKIN_MCP_TOKEN` 供后续对话复用；只有用户明确同意才可写入，禁止静默保存。
- 写入 token 前确保目录存在（`mkdir -p ~/.luckin-coffee`）；写入后建议限制权限（如 `chmod 600 ~/.luckin-coffee/LUCKIN_MCP_TOKEN`）。
- 用户要求撤销保存时，删除本地 token 文件 `~/.luckin-coffee/LUCKIN_MCP_TOKEN`，并明确告知“后续将不再从本地文件复用 token”。
...
优先调用当前智能体已配置的 `luckin-coffee` MCP 工具。若当前智能体没有配置 MCP server，但可拿到有效 token（环境变量 / 用户在对话中提供 / 本地文件 `~/.luckin-coffee/LUCKIN_MCP_TOKEN`），使用 `curl` 调用 MCP HTTP 接口。

建议先组装 token：

```bash
TOKEN="${LUCKIN_MCP_TOKEN:-}"
if [ -z "$TOKEN" ] && [ -f ~/.luckin-coffee/LUCKIN_MCP_TOKEN ]; then
  TOKEN="$(cat ~/.luckin-coffee/LUCKIN_MCP_TOKEN)"
fi
```
...
2. **Token 生命周期**：
   - 读取优先级：环境变量 `LUCKIN_MCP_TOKEN` > 当前/历史对话中用户明确提供的完整 token > 本地文件 `~/.luckin-coffee/LUCKIN_MCP_TOKEN`。
...
5. **预览订单** - 用户确认后必须调用 `previewOrder`，不得直接 `createOrder`。
   - 用于获取价格、减免金额、最终价格和 `couponCodeList`。
   - 必填：`deptId`、`productList`。
   - `amount`、`productId`、`skuCode` 来自商品搜索结果。
   - `totalInitialPrice` 是订单价格，`privilegeMoney` 是减免金额，`discountPrice` 是最终应付金额。
...
6. **创建订单** - 调用 `createOrder`。
   ...
   - 文字回复展示：订单号、门店名称、商品名、数量、价格（来自 `previewOrder.totalInitialPrice`，展示为 `¥金额`）、减免金额（来自 `previewOrder.privilegeMoney`，展示为 `¥金额`）、应付金额（优先使用 `createOrder.discountPrice`，缺失时用 `previewOrder.discountPrice`，展示为 `¥金额`）
...
- token 保存询问固定话术：`是否保存 token 到 ~/.luckin-coffee/LUCKIN_MCP_TOKEN？保存后可在后续对话自动复用，无需重复提供 token。请回复：\n1. 保存\n2. 不保存`。用户确认“保存”或“不保存”后都直接继续后续流程，避免多一轮确认。
