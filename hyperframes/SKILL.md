...
version: 1.0.0
description: >
  Create HyperFrames HTML video compositions — animations, title cards, overlays, captions,
  GSAP timelines, registry blocks/components, voiceovers, audio-reactive visuals, and scene
  transitions. Use when asked to build any HTML-based video content, write GSAP animations,
  add captions or subtitles, generate TTS narration, use registry blocks (hyperframes add),
  add transitions between scenes, or create any visual motion content.
  For CLI commands (init, lint, preview, render, transcribe, tts) and the rendering workflow
  (hf-render.py), see the hyperframes-cli skill.
source: |
  Adapted from the official HyperFrames skills by HeyGen.
  Original: https://github.com/heygen-com/hyperframes/tree/main/skills/hyperframes
  Original license: Apache-2.0 (https://github.com/heygen-com/hyperframes/blob/main/LICENSE)
  Adaptation: Merged hyperframes + gsap + hyperframes-registry into a single skill,
  added rendering notes and environment check integration.
...
> ```
> This verifies `browser_use`, `ffmpeg`, and required skills are installed.
> If `hyperframes-cli` skill is missing, install it:
> ```bash
...
Full GSAP reference: **[gsap/SKILL.md](../gsap/SKILL.md)** — timelines, easing, stagger, performance, effects.

---

## Registry Blocks & Components

Install reusable blocks/components via `hyperframes add <name>` (or, without the CLI, `curl` the registry JSON and copy files manually).
...
Full registry reference: **[hyperframes-registry/SKILL.md](../hyperframes-registry/SKILL.md)**
