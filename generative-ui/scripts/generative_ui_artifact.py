...
def infer_blocks(title, text):
    ...
    return {
      ...
      'blocks': [
        ...
        {'type':'details','title':'说明','items':[{'title':'当前环境能力','content':'可生成单文件 HTML artifact 并在当前环境中预览。'},{'title':'自动触发意图','content':'当用户提到卡片、表格、记录、代码块、图表、时间线、可视化页面时，优先调用本 skill。'}]}
      ]
    }
...
def render(spec):
    hero_cards = ''
    hero = f'<div class="hero"><div class="eyebrow">Claude Artifact Style / Prototype</div><h1>{esc(spec.get("title","Artifact"))}</h1><div class="subtitle">{esc(spec.get("summary",""))}</div><div class="chips">' + ''.join(f'<span class="chip">{esc(c)}</span>' for c in spec.get('chips', [])) + '</div></div>'
...
def main():
    ...
    out_dir = os.environ.get('GEN_UI_OUT_DIR', os.getcwd())
    out = Path(args.out or os.path.join(out_dir, f'{slugify(args.title)}_artifact.html'))
