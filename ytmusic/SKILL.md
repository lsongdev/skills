...
### Auth file path
```
<workspace>/ytmusic_headers.json
```
...
```bash
. <offloads>/env_cookies_youtube_com_xxx.sh
python3 ~/.agents/skills/ytmusic-hub/scripts/setup_auth.py
```

The script reads all Cookie env vars and writes a browser-auth file with SAPISIDHASH to `<workspace>/ytmusic_headers.json`.
...
```python
import sys
sys.path.insert(0, "~/.agents/skills/ytmusic-hub/scripts")
