#!/bin/sh
# self-improving-agent 自动初始化 + 自动记录
set -e

SKILL_BASE="~/.agents/skills/self-improving-agent/data"
PUBLIC_BASE="~/.agents/skills/self-improving-agent/data/public"
...
search_all() {
  ...
  find ~/.agents/skills/self-improving-agent ${WORKSPACE:-$HOME} -type f \( -path '~/.agents/skills/self-improving-agent/data/*.md' -o -path '~/.agents/skills/self-improving-agent/data/public/*.md' -o -path '*/.learnings/*.md' \) -print0 2>/dev/null |
...
promote_to_public() {
  ...
  init
  SRC=$(find ~/.agents/skills/self-improving-agent ${WORKSPACE:-$HOME} -type f \( -path '~/.agents/skills/self-improving-agent/data/*.md' -o -path '~/.agents/skills/self-improving-agent/data/public/*.md' -o -path '*/.learnings/*.md' \) -print0 2>/dev/null | xargs -0 grep -l "\[$ENTRY_ID\]" 2>/dev/null | head -n 1)
