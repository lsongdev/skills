...
- **Environment variable**: `NOTION_API_KEY` — Notion Internal Integration Token
  - If not set, prompt user to create: Set the environment variable `NOTION_API_KEY`
...
Path: `~/.agents/skills/notion-hub/scripts/notion_hub.py`

Run: `uv run --script --cache-dir /root/.cache/uv ~/.agents/skills/notion-hub/scripts/notion_hub.py <command> [options]`

Alias for brevity:
```sh
NH="uv run --script --cache-dir /root/.cache/uv ~/.agents/skills/notion-hub/scripts/notion_hub.py"
```
...
$NH page-read <page_id> -o <workspace>/page.md  # save to file
...
# Write from file
$NH page-write <page_id> --file <workspace>/content.md
