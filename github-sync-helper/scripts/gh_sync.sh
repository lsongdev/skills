#!/bin/sh
# github-sync-helper: reusable GitHub sync workflows
...
req.add_header("User-Agent","gh-sync-helper")
if data is not None:
...
ensure_askpass() {
  need_env GITHUB_TOKEN
  ASKPASS="${WORKSPACE:-$HOME}/.git_askpass.sh"
  if [ ! -f "$ASKPASS" ]; then
    cat > "$ASKPASS" <<'EOF'
#!/bin/sh
case "$1" in
...
data=json.dumps(payload).encode("utf-8"); req=urllib.request.Request(f"https://api.github.com/repos/{owner}/{repo}/milestones",data=data,method="POST"); req.add_header("Authorization","Bearer "+tok); req.add_header("Accept","application/vnd.github+json"); req.add_header("User-Agent","gh-sync-helper"); req.add_header("Content-Type","application/json"); resp=json.load(urllib.request.urlopen(req,timeout=30)); print(resp.get("title",""))' 2>/dev/null)" || true
...
url=f"https://api.github.com/repos/{owner}/{repo}/actions/runs?"+"&".join(qs); req=urllib.request.Request(url,headers={"Authorization":"Bearer "+tok,"Accept":"application/vnd.github+json","User-Agent":"gh-sync-helper"}); d=json.load(urllib.request.urlopen(req,timeout=30)); runs=d.get("workflow_runs",[]); lines=[]
for r in runs[:10]:
